...
public class UniRPGEditorGlobal
{
	...
	private static void OnHierarchyChanged()
	{
		...
		if (currentScene != UniRPGEditorGlobal.CurrentScene())
		{
			currentScene = UniRPGEditorGlobal.CurrentScene();
		...
		{	// when unity start (or play mode starts) then playTestingMode will be 0
			...
			else if (playTestingMode == 2)
			{
				if (!EditorApplication.isPlayingOrWillChangePlaymode)
				{
					...
					if (!string.IsNullOrEmpty(prevScene)) UniRPGEditorGlobal.OpenScene(prevScene);
				}
			}
		}
	...
	public static void RunGame()
	{
		if (UniRPGEditorGlobal.LoadDatabase())
		{
			...
			EditorPrefs.SetString("UniRPG_PlayTestingModeScene", UniRPGEditorGlobal.CurrentScene());
			UniRPGEditorGlobal.SetupBuildSettingsAndGlobals(UniRPGEditorGlobal._db, false, true);
			if (UniRPGEditorGlobal.OpenScene(UniRPGEditorGlobal._db.mainScenePaths[0]))
			{
	...
	public static void Create_Scene()
	{
		...
		// open it for editing
		UniRPGEditorGlobal.OpenScene(fn);
	}
	...
	private static void CreateDatabase()
	{
		...
		if (err)
		{
			...
			EditorUtility.DisplayDialog("Error!", "An error occurred while trying to set the Database property of the main UniRPG scene. Your UniRPG install might be corrupt.", "Close");
			return;
		}
		else
		{
...
#if UNITY_4_6 || UNITY_5_2
		return EditorApplication.OpenScene(name);
#else
